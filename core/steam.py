...
def fetch_games():
    ...
    response = requests.get(GET_APP_LIST_URL, params=params).json()["response"]
...
def fetch_achievements(app_id):
    params = {"key": settings.STEAM_API_KEY, "appid": str(app_id)}

    try:
        response = requests.get(GET_SCHEMA_FOR_GAME_URL, params=params).json()["game"][
            "availableGameStats"
        ]["achievements"]
    except KeyError:
        return None
...
def fetch_player_summary(steam_id):
    ...
    response = requests.get(GET_PLAYER_SUMMARY_URL, params=params).json()["response"][
        "players"
    ][0]
...
def fetch_player_achievements(steam_id, app_id):
    params = {"key": settings.STEAM_API_KEY, "steam_id": steam_id, "appid": app_id}

    response = requests.get(GET_PLAYER_ACHEIVEMENTS_URL, params=params).json()[
        "playerstats"
    ]["achievements"]
